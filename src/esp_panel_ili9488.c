...
const lcd_init_cmd_t ili9488_vendor_specific_init_default[] = {
...
static esp_err_t ili9488_update_madctl(ili9488_panel_t *ph)
{
    esp_err_t res;
...
esp_err_t ili9488_reset(esp_lcd_panel_t *panel)
{
    log_v("panel:0x%08x", panel);
    if (panel == NULL)
        return ESP_ERR_INVALID_ARG;

    const ili9488_panel_t *ph = (ili9488_panel_t *)panel;
...
esp_err_t ili9488_init(esp_lcd_panel_t *panel)
{
    log_v("panel:0x%08x", panel);
    if (panel == NULL)
        return ESP_ERR_INVALID_ARG;
    ...
    const lcd_init_cmd_t *cmd = ili9488_vendor_specific_init_default;
    uint16_t cmds_size = sizeof(ili9488_vendor_specific_init_default) / sizeof(lcd_init_cmd_t);
...
esp_err_t ili9488_draw_bitmap(esp_lcd_panel_t *panel, int x_start, int y_start, int x_end, int y_end, const void *color_data)
{
...
esp_err_t ili9488_invert_color(esp_lcd_panel_t *panel, bool invert)
{
...
esp_err_t ili9488_mirror(esp_lcd_panel_t *panel, bool mirror_x, bool mirror_y)
{
    log_v("panel:0x%08x, mirror_x:%d, mirror_y:%d", panel, mirror_x, mirror_y);
    if (panel == NULL)
        return ESP_ERR_INVALID_ARG;

    ili9488_panel_t *ph = (ili9488_panel_t *)panel;
    ...
    return ili9488_update_madctl(ph);
}

esp_err_t ili9488_swap_xy(esp_lcd_panel_t *panel, bool swap_xy)
{
...
esp_err_t ili9488_set_gap(esp_lcd_panel_t *panel, int x_gap, int y_gap)
{
...
esp_err_t ili9488_disp_off(esp_lcd_panel_t *panel, bool off)
{
...
esp_err_t ili9488_del(esp_lcd_panel_t *panel)
{
    log_v("panel:0x%08x", panel);
    if (panel == NULL)
        return ESP_ERR_INVALID_ARG;
...
esp_err_t esp_lcd_new_panel_ili9488(const esp_lcd_panel_io_handle_t io, const esp_lcd_panel_dev_config_t *config, esp_lcd_panel_handle_t *handle)
{
    ...
    ili9488_panel_t *ph = heap_caps_calloc(1, sizeof(ili9488_panel_t), MALLOC_CAP_DEFAULT);
    if (ph == NULL)
    {
    ...
    ph->base.del = ili9488_del;
    ph->base.reset = ili9488_reset;
    ph->base.init = ili9488_init;
    ph->base.draw_bitmap = ili9488_draw_bitmap;
    ph->base.invert_color = ili9488_invert_color;
    ph->base.mirror = ili9488_mirror;
    ph->base.swap_xy = ili9488_swap_xy;
    ph->base.set_gap = ili9488_set_gap;
    ph->base.disp_off = ili9488_disp_off;
