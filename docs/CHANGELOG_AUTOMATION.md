...
The GitHub Actions release workflow (`build-esp32.yml`) now automatically extracts changelog content from `CHANGELOG.md` and includes it in the release body when creating a new release.
...
2. **Changelog Parsing**: An AWK script searches `CHANGELOG.md` for the matching version section
...
3. **Release Creation**: The extracted changelog content is included as the release body
   - If no matching version is found, a fallback message is used with a link to CHANGELOG.md
...
This step was added to the release job to ensure CHANGELOG.md is available.
...
- name: Extract changelog for this release
  ...
  run: |
    ...
    if [ -z "$CHANGELOG_CONTENT" ]; then
      CHANGELOG_CONTENT="See [CHANGELOG.md](https://github.com/${{ github.repository }}/blob/${{ github.ref_name }}/CHANGELOG.md) for details."
    fi
...
For automatic extraction to work correctly, `CHANGELOG.md` must follow this format:
...
For tag `v0.0.3`, the workflow will extract this section from CHANGELOG.md:
...
If the version is not found in CHANGELOG.md, the release will include:

```
See [CHANGELOG.md](https://github.com/julesdg6/aCYD-MIDI/blob/vX.Y.Z/CHANGELOG.md) for details.
```
