...
Copy the root template and update it with your credentials:

```bash
cp config/wifi_config.local.h.template config/wifi_config.local.h
```

Then edit `config/wifi_config.local.h` and update:
...
**Note:** The `config/wifi_config.local.h` file is ignored by git to prevent accidentally committing credentials.
...
1. Edit `config/wifi_config.local.h`:
   ```cpp
